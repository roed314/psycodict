 ...
 * **`meta_version`** — a single-row stamp of the metadata *format* version (currently `META_VERSION = 1`).  Every connection reads it and refuses to run (raising `RuntimeError`) if it does not match the version this psycodict expects: a newer stamp means "upgrade psycodict", an older one means "migrate the meta tables".  A database that has meta tables but no `meta_version` is treated as the pre-stamp format `1`; a database with no meta tables at all is fresh and must be connected to with `PostgresDatabase(create=True)`, which bootstraps all of the above.
...
`reload(searchfile, countsfile=None, statsfile=None, indexesfile=None, constraintsfile=None, metafile=None, ...)` is the safe way to *replace* a table's contents.  It is a **clone-build-swap**:
 ...
 1. clone the live table's schema into `name_tmp` (via `CREATE TABLE ... (LIKE ...)`, which copies columns and defaults but no indexes, constraints or primary key);
